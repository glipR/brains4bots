...
But currently her code has two problems (errors in programs are often called bugs). Can you spot them?
...
<div class="hint" key="bugs-2" title="Hint 2" markdown="1">After fixing the first problem, the code will run, but the second line printed will not be `30 + 3 = my_number`. Why is this?</div>
</div>
</div>
...
y = 3
print(x + y)
# The * is for multiplication. You can write it with Shift and 8 on the keyboard
...
<div class="note" title="Blocking operations" open="1">
...
Using the bot you've created, write some code to move around the field in a square pattern, hitting all 4 locations.

You can download the task [here](ev3simc://drive.google.com/uc?export=download&id=1oBxGOzqUwD-R3_2Gs2X0V9C9DECMsmEH).

Here's an example of what you need to make the motors do:

![Example of task being completed](/assets/img/square.gif)

<div>
<div class="hint" key="square-1" title="Hint">You can place the motors the same way as the video above to make the code a bit easier. One motor controls left/right movement and the other controls up/down.</div>
</div>
</div>
